...
class rollbar_notifier(object):
    ...
    def __init__(self, rollbar_key, rollbar_env, rollbar_level, logger):
        ...
        Class constructor.
        """
        self.levels = {"critical": 1, "error": 2, "warning": 3, "info": 5}
...
class replica_engine(object):
    ...
    def __init__(self, args):
        ...
        self.notifier = rollbar_notifier(
            self.config["rollbar_key"],
            self.config["rollbar_env"],
            self.args.rollbar_level,
            self.logger,
        )
        ...
        self.mysql_source.convert_columns = self.config["convert_columns"]
        ...
        if self.args.command == "upgrade_replica_schema":
            ...
            self.args.command == "enable_replica"
            and self.catalog_version != catalog_version
        ):
            print(
    ...
    def terminate_replica(self, signal, frame):
        ...
        """
        self.logger.info(
            "Caught stop replica signal terminating daemons and ending the replica process."
        )
    ...
    def set_configuration_files(self):
        ...
        if os.path.isfile(self.local_conf_example):
            if os.path.getctime(self.global_conf_example) > os.path.getctime(
                self.local_conf_example
            ):
                print(
                    "updating configuration example with %s" % self.local_conf_example
                )
    ...
    def load_config(self):
        ...
        if not os.path.isfile(self.config_file):
            print(
                "**FATAL - configuration file missing. Please ensure the file %s is present."
                % (self.config_file)
            )
            sys.exit()
    ...
    def show_sources(self):
        ...
        for item in self.config["sources"]:
            ...
            connection_list = [
                param for param in source["db_conn"] if param not in ["password"]
            ]
    ...
    def show_config(self):
        ...
        config_list = [
            ...
            if item not in ["pg_conn", "sources", "type_override"]
        ]
        connection_list = [
            item for item in self.config["pg_conn"] if item not in ["password"]
        ]
    ...
    def drop_source(self):
        ...
        if self.args.source == "*":
            ...
            drp_msg = (
                "Dropping the source %s will remove drop any replica reference.\n Are you sure? YES/No\n"
                % self.args.source
            )
    ...
    def __init_mysql_replica(self):
        ...
        if self.args.debug:
            ...
            self.logger.info(
                "Initialising the replica for source %s" % self.args.source
            )
            init_daemon = Daemonize(
                app="init_replica",
                pid=init_pid,
                action=self.mysql_source.init_replica,
        ...
        if self.args.source == "*":
            ...
            if self.args.debug:
                ...
                if self.config["log_dest"] == "stdout":
                    ...
                    foreground = False
                    print(
                        "Sync tables process for source %s started."
                        % (self.args.source)
                    )
                keep_fds = [self.logger_fds]
                init_pid = os.path.expanduser(
                ...
                sync_daemon = Daemonize(
                    app="sync_tables",
                    pid=init_pid,
                    action=self.mysql_source.refresh_schema,
                    foreground=foreground,
                    keep_fds=keep_fds,
                )
                sync_daemon.start()
    ...
    def upgrade_replica_schema(self):
        ...
        if catalog_version == self.catalog_version:
            ...
            if catalog_version == self.upgradable_version:
                upg_msg = (
                    "Upgrading the catalogue %s to the version %s.\n Are you sure? YES/No\n"
                ...
                catalog_version.split(".")[0] == "2"
                and catalog_version.split(".")[1] == "0"
            ):
                ...
                print(
                    "Wrong starting version. Expected %s, got %s"
                    % (catalog_version, self.upgradable_version)
                )
                sys.exit()
    ...
    def read_replica(self, queue, log_read):
        ...
        if "keep_existing_schema" in self.config["sources"][self.args.source]:
            keep_existing_schema = self.config["sources"][self.args.source][
        ...
        while True:
            try:
                ...
                if len(tables_error) > 0:
                    ...
                    tables_removed = "\n".join(table_list)
                    notifier_message = (
                        "There was an error during the replay of data. %s. The affected tables are no longer replicated."
                        % (tables_removed)
                    )
    ...
    def __run_replica(self):
        ...
        if "auto_maintenance" not in self.config["sources"][self.args.source]:
            ...
            auto_maintenance = self.config["sources"][self.args.source][
                "auto_maintenance"
            ]
        ...
        self.logger.info(
            "Starting the replica daemons for source %s " % (self.args.source)
        )
        self.read_daemon = mp.Process(
            target=self.read_replica,
            name="read_replica",
            daemon=True,
            args=(
                queue,
        ...
        self.replay_daemon = mp.Process(
            target=self.replay_replica,
            name="replay_replica",
            daemon=True,
            args=(
                queue,
        ...
        while True:
            ...
            if read_alive and replay_alive:
                self.logger.debug(
                    "Replica process for source %s is running" % (self.args.source)
                )
                self.pg_engine.cleanup_replayed_batches()
            else:
                stack_trace = queue.get()
                self.logger.error(
                    "Read process alive: %s - Replay process alive: %s"
                    % (
                        read_alive,
                        replay_alive,
                    )
                ...
                if read_alive:
                    self.read_daemon.terminate()
                    self.logger.error(
                        "Replay daemon crashed. Terminating the read daemon."
                    )
                if replay_alive:
                    self.replay_daemon.terminate()
                    self.logger.error(
                        "Read daemon crashed. Terminating the replay daemon."
                    )
                if self.args.debug:
    ...
    def start_replica(self):
        ...
        if self.args.source == "*":
            ...
            self.pg_engine.connect_db()
            self.logger.info(
                "Checking if the replica for source %s is stopped " % (self.args.source)
            )
            ...
            if replica_status in ["syncing", "running", "initialising"]:
                ...
                print(
                    "chameleon.py enable_replica --config %s --source %s "
                    % (self.args.config, self.args.source)
                )

            else:
                self.logger.info(
                    "Cleaning not processed batches for source %s" % (self.args.source)
                )
                ...
                if self.args.debug:
                    ...
                    if self.config["log_dest"] == "stdout":
                        ...
                        foreground = False
                        print(
                            "Starting the replica process for source %s"
                            % (self.args.source)
                        )

                    keep_fds = [self.logger_fds]
                    app_name = "%s_replica" % self.args.source
                    replica_daemon = Daemonize(
                        app=app_name,
                        pid=replica_pid,
    ...
    def __stop_replica(self):
        ...
        """
        replica_pid = os.path.expanduser(
            "%s/%s.pid" % (self.config["pid_dir"], self.source)
        )
    ...
    def show_status(self):
        ...
        if self.args.source != "*":
            if "auto_maintenance" in self.config["sources"][self.args.source]:
                self.pg_engine.auto_maintenance = self.config["sources"][
                    self.args.source
                ]["auto_maintenance"]
    ...
    def detach_replica(self):
        ...
        if self.args.source == "*":
            ...
            drp_msg = (
                "Detaching the replica will remove any reference for the source %s.\n Are you sure? YES/No\n"
                % self.args.source
            )
            drop_src = input(drp_msg)
            if drop_src == "YES":
                ...
                if not keep_existing_schema:
                    self.pg_engine.fk_metadata = (
                        self.mysql_source.get_foreign_keys_metadata()
                    )
                self.__stop_replica()
    ...
    def run_maintenance(self):
        ...
        """
        maintenance_pid = os.path.expanduser(
            "%s/%s_maintenance.pid" % (self.config["pid_dir"], self.args.source)
        )
        if self.args.source == "*":
            print("You must specify a source name with the argument --source")
        else:
            if self.args.debug:
                ...
                if self.config["log_dest"] == "stdout":
                    ...
                    self.logger.info(
                        "Starting the maintenance on the source %s"
                        % (self.args.source,)
                    )
                    foreground = False
                    print(
                        "Starting the maintenance process for source %s"
                        % (self.args.source)
                    )
                    keep_fds = [self.logger_fds]

                    app_name = "%s_maintenance" % self.args.source
                    maintenance_daemon = Daemonize(
                        app=app_name,
                        pid=maintenance_pid,
                        action=self.pg_engine.run_maintenance,
                        foreground=foreground,
                        keep_fds=keep_fds,
                    )
                    try:
                        ...
                        print(
                            "The  maintenance process is already started. Aborting the command."
                        )
    ...
    def __init_logger(self, logger_name):
        ...
        if log_dest == "stdout" or debug_mode:
            ...
            fh = TimedRotatingFileHandler(
                log_file, when="d", interval=1, backupCount=log_days_keep
            )
