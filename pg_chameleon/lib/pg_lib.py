...
class pgsql_source(object):
    ...
    def __connect_db(self, auto_commit=True, dict_cursor=False):
        ...
        if self.source_conn:
            strconn = (
                "dbname=%(database)s user=%(user)s host=%(host)s password=%(password)s port=%(port)s connect_timeout=%(connect_timeout)s"
                % self.source_conn
            )
        ...
        for schema in self.schema_list:
            ...
            try:
                ...
                if len(limit_tables) > 0:
                    table_list = [
                        table for table in table_list if table in limit_tables
                    ]
            except KeyError:
                pass
            try:
                ...
                if len(skip_tables) > 0:
                    table_list = [
                        table for table in table_list if table not in skip_tables
                    ]
            except KeyError:
                pass
    ...
    def __create_destination_schemas(self):
        ...
        for schema in self.schema_list:
            ...
            self.schema_loading[schema] = {
                "destination": destination_schema,
                "loading": loading_schema,
            }
    ...
    def __copy_data(self, schema, table, db_copy):
        ...
        copy_file.close()
        self.logger.debug(
            "loading the file %s in table %s.%s "
            % (
                out_file,
                loading_schema,
                table,
    ...
    def __create_indices(self):
        ...
        for schema in self.schema_tables:
            ...
            for table in table_list:
                ...
                self.pg_engine.pgsql_cur.execute(
                    "SET search_path=%s;", (loading_schema,)
                )
                ...
                for idx in idx_tab:
                    ...
                    try:
                        ...
                        self.logger.error(
                            "an error occcurred when executing %s" % (idx[0])
                        )
                    if idx[2]:
                        self.pg_engine.store_table(
                            destination_schema, table, ["foo"], None
                        )
    ...
    def init_replica(self):
        ...
        try:
            ...
            self.pg_engine.set_source_highwatermark(
                fake_master, consistent=self.consistent
            )
...
class pg_engine(object):
    ...
    def connect_db(self):
        ...
        if self.dest_conn and not self.pgsql_conn:
            strconn = (
                "dbname=%(database)s user=%(user)s host=%(host)s password=%(password)s port=%(port)s"
                % self.dest_conn
            )
    ...
    def set_lock_timeout(self):
        ...
        """
        self.logger.debug(
            "Changing the lock timeout for the session to %s." % self.lock_timeout
        )
    ...
    def create_foreign_keys(self):
        ...
        for foreign_key in self.fk_metadata:
            ...
            referenced_table_schema = schema_mappings[
            ...
            fk_counter += 1
            self.logger.info(
                "creating invalid foreign key %s on table %s.%s"
                % (fk_name, table_schema, table_name)
            )
            try:
                self.pgsql_cur.execute(sql_fkey)
            except psycopg2.Error as e:
                self.logger.error(
                    "could not create the foreign key %s on table %s.%s"
                    % (fk_name, table_schema, table_name)
                )
                self.logger.error("SQLCODE: %s SQLERROR: %s" % (e.pgcode, e.pgerror))
        ...
        for fkey in fk_list:
            self.logger.info(
                "validating %s on table %s.%s"
                % (fkey["fkey_name"], fkey["table_schema"], fkey["table_name"])
            )
            ...
            try:
                ...
                self.logger.error(
                    "could not validate the foreign key %s on table %s"
                    % (fkey["table_name"], fkey["fkey_name"])
                )
                self.logger.error("SQLCODE: %s SQLERROR: %s" % (e.pgcode, e.pgerror))
                ...
                for db_role in self.grant_select_to:
                    ...
                    try:
                        ...
                        for table in self.schema_tables[schema]:
                            self.logger.info(
                                "Granting select on table %s.%s to the role %s."
                                % (schema_loading, table, db_role)
                            )
                            sql_grant_select = sql.SQL(
                            ...
                            try:
                                ...
                                self.logger.error(
                                    "SQLCODE: %s SQLERROR: %s" % (er.pgcode, er.pgerror)
                                )
                    except psycopg2.Error as e:
                        if e.pgcode == "42704":
                            self.logger.warning(
                                "The role %s does not exist" % (db_role,)
                            )
                        else:
                            self.logger.error(
                                "SQLCODE: %s SQLERROR: %s" % (e.pgcode, e.pgerror)
                            )
    ...
    def set_read_paused(self, read_paused):
        ...
        sql_pause = """
            ...
            ;
        """
        self.pgsql_cur.execute(
            sql_pause, (read_paused, self.i_id_source, not_read_paused)
        )
    ...
    def __vacuum_full_log_tables(self):
        ...
        for sql_stat in vacuum_sql:
            ...
            try:
                ...
                self.logger.error(
                    "An error occurred when running VACUUM FULL on the table %s"
                    % (sql_stat[0])
                )
    ...
    def __vacuum_log_tables(self):
        ...
        for sql_stat in vacuum_sql:
            ...
            try:
                ...
                self.logger.error(
                    "An error occurred when running VACUUM on the table %s"
                    % (sql_stat[0])
                )
    ...
    def run_maintenance(self):
        ...
        if check_maintenance:
            self.logger.info(
                "The source is already in maintenance. Skipping the maintenance run."
            )
        else:
    ...
    def replay_replica(self):
        ...
        if replica_paused:
            ...
            exit_on_error = (
                True if self.source_config["on_error_replay"] == "exit" else False
            )
            while continue_loop:
                sql_replay = (
                    """SELECT * FROM sch_chameleon.fn_replay_mysql(%s,%s,%s);"""
                )
                self.pgsql_cur.execute(
                    sql_replay, (replay_max_rows, self.i_id_source, exit_on_error)
                )
                replay_status = self.pgsql_cur.fetchone()
                if replay_status[0]:
                    self.logger.info(
                        "Replayed at most %s rows for source %s"
                        % (replay_max_rows, self.source)
                    )
                replica_paused = self.get_replica_paused()
                if replica_paused:
                    break
    ...
    def cleanup_replayed_batches(self):
        ...
        self.logger.debug(
            "Cleaning replayed batches for source %s older than %s"
            % (self.source, batch_retention)
        )
        sql_cleanup = """
            DELETE FROM
    ...
    def __generate_ddl(self, token, destination_schema):
        ...
        if token["command"] == "CREATE TABLE":
            ...
            table_ddl = self.__build_create_table_mysql(
                table_metadata, table_name, destination_schema, temporary_schema=False
            )
            table_enum = "".join(table_ddl["enum"])
            table_statement = table_ddl["table"]
            index_ddl = self.build_create_index(
                destination_schema, table_name, index_data
            )
            ...
            if count_table == 1:
                if token["command"] == "RENAME TABLE":
                    ...
                    query = """ALTER TABLE "%s"."%s" RENAME TO "%s" """ % (
                        destination_schema,
                        old_name,
                        new_name,
                    )
                    ...
                    query = """ TRUNCATE TABLE "%s"."%s" CASCADE;""" % (
                        destination_schema,
                        token["name"],
                    )
    ...
    def build_enum_ddl(self, schema, enm_dic):
        ...
        if type_data:
            if type_data[0] == "E" and enm_dic["type"] == "enum":
                ...
                new_enums = [
                    val.strip()
                    for val in enm_dic["enum_list"]
                    if val.strip() not in type_data[3]
                ]
                sql_add = []
                for enumeration in new_enums:
                    sql_add = """ALTER TYPE "%s"."%s" ADD VALUE '%s';""" % (
                        type_data[2],
                        enum_name,
                        enumeration,
                    )
                ...
                self.logger.debug(
                    "The column will be altered in enum, creating the type"
                )
                pre_alter = """CREATE TYPE "%s"."%s" AS ENUM (%s);""" % (
                    schema,
                    enum_name,
                    enm_dic["enum_elements"],
                )
    ...
    def build_alter_table(self, schema, token):
        ...
        for alter_dic in token["alter_cmd"]:
            if alter_dic["command"] == "DROP":
                ...
                alter_cmd.append(
                    '%s "%s" %s NULL %s'
                    % (alter_dic["command"], column_name, column_type, default_value)
                )
                ...
                column_type = self.get_data_type(alter_dic, schema, table_name)
                default_sql = self.generate_default_statements(
    ...
    def __drop_primary_key(self, schema, token):
        ...
        """
        self.logger.info(
            "dropping primary key for table %s.%s"
            % (
                schema,
                token["name"],
            )
    ...
    def upgrade_catalogue_v20(self):
        ...
        if sources_active[0] == 0:
            ...
            catalog_number = int(
                "".join([value for value in catalog_version.split(".")])
            )
            self.connect_db()
            for migration in self.migrations:
                migration_version = migration["version"]
                migration_number = int(
                    "".join([value for value in migration_version.split(".")])
                )
                if migration_number > catalog_number:
                    migration_file_name = "%s/%s" % (
                        self.sql_upgrade_dir,
                        migration["script"],
                    )
                    print(
                        "Migrating the catalogue from version %s to version %s"
                        % (catalog_version, migration_version)
                    )
    ...
    def upgrade_catalogue_v1(self):
        ...
        if source_replay:
            for source in source_replay:
                ...
                self.logger.info(
                    "Replaying last %s rows for source %s " % (replay_rows, source_name)
                )
                continue_loop = True
                while continue_loop:
                    ...
                    if continue_loop:
                        self.logger.info(
                            "Still replaying rows for source %s" % (source_name,)
                        )
        ...
        for source in self.sources:
            ...
            if not source_mapped:
                self.logger.error(
                    "Checks for source %s failed. Matched mappings %s, configured mappings %s"
        ...
        if upgrade_possible:
            try:
                self.logger.info(
                    "Renaming the old schema %s in %s "
                    % (self.__v2_schema, self.__v1_schema)
                )
                sql_rename_old = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
            ...
            self.logger.error(
                "Sanity checks for the schema mappings failed. Aborting the upgrade"
            )
            self.rollback_upgrade_v1()
        self.disconnect_db()
    ...
    def rollback_upgrade_v1(self):
        ...
        if v1_schema[0] == 1:
            self.logger.info(
                "The schema %s exists, rolling back the changes" % (self.__v1_schema)
            )
            self.pgsql_cur.execute(sql_check, (self.__current_schema,))
            curr_schema = self.pgsql_cur.fetchone()
            if curr_schema[0] == 1:
                self.logger.info(
                    "Renaming the current schema %s in %s"
                    % (self.__current_schema, self.__v2_schema)
                )
                sql_rename_current = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
                    sql.Identifier(self.__current_schema),
                    sql.Identifier(self.__v2_schema),
                )
                self.pgsql_cur.execute(sql_rename_current)
            sql_rename_old = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
                sql.Identifier(self.__v1_schema), sql.Identifier(self.__current_schema)
            )
            self.pgsql_cur.execute(sql_rename_old)
        else:
            self.logger.info(
                "The old schema %s does not exists, aborting the rollback"
                % (self.__v1_schema)
            )
            sys.exit()
        self.logger.info(
    ...
    def unregister_table(self, schema, table):
        ...
        """
        self.logger.info(
            "unregistering table %s.%s from the replica catalog"
            % (
                schema,
                table,
            )
    ...
    def cleanup_source_tables(self):
        ...
        """
        self.logger.info(
            "deleting all the table references from the replica catalog for source %s "
            % (self.source,)
        )
        sql_delete = """ DELETE FROM sch_chameleon.t_replica_tables
                    WHERE
    ...
    def cleanup_table_events(self):
        ...
        sql_cleanup = (
            "DELETE FROM sch_chameleon.{} WHERE format('%%I.%%I',v_schema_name,v_table_name) IN (%s) ;"
    ...
    def get_tables_disabled(self, format="csv"):
        ...
        if format == "csv":
            ...
            select_clause = (
                """array_agg(format('%s.%s',(t_mappings).key,v_table_name)) """
            )
        sql_get = (
            """
            SELECT
    ...
    def add_source(self):
        ...
        if num_sources == 0:
            ...
            if check_mappings:
                self.logger.error(
                    "Could not register the source %s. There is a duplicate destination schema in the schema mappings."
                    % self.source
                )
            else:
                self.logger.debug("Adding source %s " % self.source)
                schema_mappings = json.dumps(
                    self.sources[self.source]["schema_mappings"]
                )
        ...
        if num_sources == 1:
            ...
            for log_table in source_drop[0]:
                ...
                try:
                    ...
                    self.logger.debug(
                        "Could not drop the table sch_chameleon.%s you may need to remove it manually."
                        % log_table
                    )
        else:
            self.logger.debug(
                "There is no source %s registered in the replica catalogue"
                % self.source
            )
    ...
    def get_schema_list(self):
        ...
        self.logger.debug(
            "Found origin's replication schemas %s" % ", ".join(schema_list)
        )
        return schema_list

    def __build_create_table_pgsql(
        self, table_metadata, table_name, schema, temporary_schema=True
    ):
        """
        ...
        for column in table_metadata:
            ...
            if column_type == "enum":
                enum_type = '"%s"."enum_%s_%s"' % (
                    destination_schema,
                    table_name[0:20],
                    column["column_name"][0:20],
                )
                sql_drop_enum = "DROP TYPE IF EXISTS %s CASCADE;" % enum_type
                sql_create_enum = "CREATE TYPE %s AS ENUM (%s);" % (
                    enum_type,
                    column["typ_elements"],
                )
            ...
            if column_type == "composite":
                ...
                sql_create_composite = "CREATE TYPE %s AS (%s);" % (
                    composite_type,
                    column["typ_elements"],
                )
            ...
            if column["col_serial"]:
                ...
                default_value = ""
            ddl_columns.append(
                '"%s" %s %s %s' % (column_name, column_type, default_value, col_is_null)
            )
    ...
    def __build_create_table_mysql(
        self, table_metadata, table_name, schema, temporary_schema=True
    ):
        """
        ...
        for column in table_metadata:
            ...
            if column_type == "numeric":
                column_type = "%s (%s,%s)" % (
                    column_type,
                    str(column["numeric_precision"]),
                    str(column["numeric_scale"]),
                )
            if column["extra"] == "auto_increment":
                column_type = "bigserial"
            ddl_columns.append(
                ' "%s" %s %s   ' % (column["column_name"], column_type, col_is_null)
            )
        def_columns = str(",").join(ddl_columns)
        table_ddl["enum"] = ddl_enum
        table_ddl["composite"] = []
    ...
    def get_status(self):
        ...
        if self.source == "*":
            ...
            source_filter = (
                self.pgsql_cur.mogrify(""" WHERE  src.t_source=%s """, (self.source,))
            ).decode()
            self.set_source_id()
    ...
    def generate_default_statements(self, schema, table, column, create_column=None):
        ...
        return {
            "drop": query_drop_default.decode(),
            "create": query_add_default.decode(),
        }
    ...
    def insert_batch(self, group_insert):
        ...
        for row_data in group_insert:
            ...
            try:
                ...
                if e.pgcode == "22P05":
                    self.logger.warning(
                        "%s - %s. Trying to cleanup the row" % (e.pgcode, e.pgerror)
                    )
                    ...
                    try:
                        ...
                        self.logger.error(
                            "Cleanup unsuccessful. Saving the discarded row"
                        )
                    ...
                    self.logger.error(
                        "Error when storing event data. Saving the discarded row"
                    )
                    self.save_discarded_row(row_data)
            except:
                self.logger.error(
    ...
    def create_table(self, table_metadata, table_name, schema, metadata_type):
        ...
        if metadata_type == "mysql":
            table_ddl = self.__build_create_table_mysql(
                table_metadata, table_name, schema
            )
        elif metadata_type == "pgsql":
            table_ddl = self.__build_create_table_pgsql(
                table_metadata, table_name, schema
            )
    ...
    def update_schema_mappings(self):
        ...
        if new_schema_mappings != old_schema_mappings:
            ...
            if not duplicate_mappings:
                self.logger.debug(
                    "Updating schema mappings for source %s" % self.source
                )
                ...
                for schema in old_schema_mappings:
                    ...
                    if not new_mapping:
                        ...
                        """
                        self.pgsql_cur.execute(
                            sql_delete, (self.i_id_source, old_mapping)
                        )
                    elif old_mapping != new_mapping:
                        self.logger.debug(
                            "Updating mapping for schema %s. Old: %s. New: %s"
                            % (schema, old_mapping, new_mapping)
                        )
                        sql_tables = """
                            UPDATE sch_chameleon.t_replica_tables
                                SET v_schema_name=%s
                        ...
                        """
                        self.pgsql_cur.execute(
                            sql_tables, (new_mapping, self.i_id_source, old_mapping)
                        )
                        sql_alter_schema = sql.SQL(
                            "ALTER SCHEMA {} RENAME TO {};"
                        ).format(
                            sql.Identifier(old_mapping), sql.Identifier(new_mapping)
                        )
                ...
                """
                self.pgsql_cur.execute(
                    sql_source, (json.dumps(new_schema_mappings), self.i_id_source)
                )
            ...
            self.logger.debug(
                "The configuration file and catalogue mappings for source %s are the same. Not updating."
                % self.source
            )
    ...
    def check_source_consistent(self):
        ...
        if source_consistent:
            if source_consistent[0]:
                self.logger.info(
                    "The source: %s reached the consistent status" % (self.source,)
                )
    ...
    def reindex_table(self, schema, table):
        ...
        :param table: the table's name
        """
        sql_reindex = sql.SQL("REINDEX TABLE {}.{} ;").format(
    ...
    def cleanup_idx_cons(self, schema, table):
        ...
        for fk in fk_drop:
            self.logger.info(
                "Dropping the foreign key {}".format(
                    fk[0],
                )
            )
            try:
        ...
        for pk in pk_drop:
            self.logger.info(
                "Dropping the primary key {}".format(
                    pk[0],
                )
            )
            try:
    ...
    def __create_foreign_keys(self):
        ...
        for fk in fk_create:
            self.logger.info(
                "Creating the foreign key {}".format(
                    fk[0],
                )
            )
            try:
    ...
    def create_idx_cons(self, schema, table):
        ...
        for pk in pk_create:
            self.logger.info(
                "Creating the primary key {}".format(
                    pk[0],
                )
            )
            self.pgsql_cur.execute(pk[1])
    ...
    def collect_idx_cons(self, schema, table):
        ...
        """
        self.logger.info(
            "Collecting indices for the table %s.%s"
            % (
                schema,
                table,
            )
        ...
        self.logger.info(
            "Collecting the primary key for the table %s.%s"
            % (
                schema,
                table,
            )
        ...
        self.logger.info(
            "Collecting unique constraints for the table %s.%s"
            % (
                schema,
                table,
            )
        ...
        self.logger.info(
            "Collecting foreign keys for the table %s.%s"
            % (
                schema,
                table,
            )
    ...
    def truncate_table(self, schema, table):
        ...
        :param table: the table's name
        """
        sql_truncate = sql.SQL("TRUNCATE TABLE {}.{};").format(
            sql.Identifier(schema), sql.Identifier(table)
        )
    ...
    def store_table(self, schema, table, table_pkey, master_status):
        ...
        if len(table_pkey) > 0:
            ...
            self.pgsql_cur.execute(
                sql_insert,
                (self.i_id_source, table, schema, table_pkey, binlog_file, binlog_pos),
            )
        else:
            self.logger.warning(
                "Missing primary key. The table %s.%s will not be replicated."
                % (
                    schema,
                    table,
                )
    ...
    def insert_data(self, schema, table, insert_data, column_list):
        ...
        sql_head = 'INSERT INTO "%s"."%s"(%s) VALUES (%s);' % (
            schema,
            table,
            column_list,
            column_marker,
        ...
        for data_row in insert_data:
            try:
                ...
                self.logger.warning(
                    "character mismatch when inserting the data, trying to cleanup the row data"
                )
                cleanup_data_row = []
    ...
    def create_indices(self, schema, table, index_data):
        ...
        for index in index_data:
            ...
            if indx == "PRIMARY":
                ...
                pkey_def = 'ALTER TABLE "%s"."%s" ADD PRIMARY KEY (%s) ;' % (
                    schema,
                    table,
                    ",".join(index_columns),
                )
    ...
    def swap_schemas(self):
        ...
        for schema in self.schema_loading:
            ...
            sql_dest_to_tmp = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
                sql.Identifier(schema_destination), sql.Identifier(schema_temporary)
            )
            sql_load_to_dest = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
                sql.Identifier(schema_loading), sql.Identifier(schema_destination)
            )
            sql_tmp_to_load = sql.SQL("ALTER SCHEMA {} RENAME TO {};").format(
                sql.Identifier(schema_temporary), sql.Identifier(schema_loading)
            )
            self.logger.info(
                "Swapping schema %s with %s" % (schema_destination, schema_loading)
            )
            self.logger.debug(
                "Renaming schema %s in %s" % (schema_destination, schema_temporary)
            )
            self.pgsql_cur.execute(sql_dest_to_tmp)
            self.logger.debug(
                "Renaming schema %s in %s" % (schema_loading, schema_destination)
            )
            self.pgsql_cur.execute(sql_load_to_dest)
            self.logger.debug(
                "Renaming schema %s in %s" % (schema_temporary, schema_loading)
            )
    ...
    def __swap_enums(self):
        ...
        for schema in self.schema_tables:
            ...
            for enumeration in enum_list:
                ...
                sql_set_schema_new = sql.SQL("ALTER TYPE {}.{} SET SCHEMA {};").format(
                    sql.Identifier(schema_loading),
                    sql.Identifier(type_name),
                    sql.Identifier(schema_destination),
                )
                self.logger.debug(
                    "Dropping the original tpye %s.%s "
                    % (schema_destination, type_name)
                )
                self.pgsql_cur.execute(sql_drop_origin)
                self.logger.debug(
                    "Changing the schema for type %s.%s to %s"
                    % (schema_loading, type_name, schema_destination)
                )
    ...
    def swap_tables(self):
        ...
        for schema in self.schema_tables:
            ...
            for table in self.schema_tables[schema]:
                self.logger.info(
                    "Swapping table %s.%s with %s.%s"
                    % (schema_destination, table, schema_loading, table)
                )
                sql_drop_origin = sql.SQL("DROP TABLE IF EXISTS {}.{} ;").format(
                    sql.Identifier(schema_destination), sql.Identifier(table)
                )
                sql_set_schema_new = sql.SQL("ALTER TABLE {}.{} SET SCHEMA {};").format(
                    sql.Identifier(schema_loading),
                    sql.Identifier(table),
                    sql.Identifier(schema_destination),
                )
                self.logger.debug(
                    "Dropping the original table %s.%s " % (schema_destination, table)
                )
                self.pgsql_cur.execute(sql_drop_origin)
                self.logger.debug(
                    "Changing the schema for table %s.%s to %s"
                    % (schema_loading, table, schema_destination)
                )
                self.pgsql_cur.execute(sql_set_schema_new)
                self.pgsql_conn.commit()
    ...
    def create_database_schema(self, schema_name):
        ...
        """
        sql_create = sql.SQL("CREATE SCHEMA IF NOT EXISTS {};").format(
    ...
    def drop_database_schema(self, schema_name, cascade):
        ...
        try:
            ...
            self.logger.error(
                "could not drop the schema %s. You will need to drop it manually."
                % schema_name
            )
