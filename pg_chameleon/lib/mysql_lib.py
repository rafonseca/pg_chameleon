...
class mysql_source(object):
    ...
    def __check_mysql_config(self):
        ...
        if (
            (log_bin.upper() == "ON" or skip_log_bin_check)
            and binlog_format.upper() == "ROW"
            and binlog_row_image.upper() == "FULL"
        ):
            ...
            self.logger.error(
                "The MySQL configuration does not allow the replica. Exiting now"
            )
            self.logger.error(
                "Source settings - log_bin %s, binlog_format %s, binlog_row_image %s"
                % (log_bin.upper(), binlog_format.upper(), binlog_row_image.upper())
            )
            self.logger.error(
                "Mandatory settings - log_bin ON, binlog_format ROW, binlog_row_image FULL (only for MySQL 5.6+) "
            )
            sys.exit()
        ...
        if self.tables != "*":
            ...
            if limit_tables:
                ...
                limit_tables = [
                    table
                    for table in tables
                    if table in limit_tables or table.split(".")[0] not in limit_schemas
                ]
            else:
                limit_tables = tables
    ...
    def get_table_list(self):
        ...
        for schema in self.schema_list:
            ...
            table_list = [
                table["table_name"] for table in self.cursor_buffered.fetchall()
            ]
            try:
                selected_tables = set(self.limit_tables.get(schema,table_list)) - set(
                    self.skip_tables.get(schema, [])
                )
                if selected_tables:  # should restrict table_list
                    table_list = [
                        table for table in table_list if table in selected_tables
                    ]
    ...
    def create_destination_schemas(self):
        ...
        if self.keep_existing_schema:
            ...
            for schema in self.schema_list:
                ...
                self.logger.debug(
                    "Creating the destination schema %s." % destination_schema
                )
    ...
    def get_foreign_keys_metadata(self):
        ...
        """
        self.__init_sync()
        schema_replica = "'%s'" % "','".join(
            [schema.strip() for schema in self.sources[self.source]["schema_mappings"]]
        )
        self.logger.info(
            "retrieving foreign keys metadata for schemas %s" % schema_replica
        )
        sql_fkeys = """
            SELECT
    ...
    def generate_select_statements(self, schema, table):
        ...
        sql_select = (
            ...
            + """')
                    ...
                    WHEN
                        data_type IN ('"""
            + "','".join(self.spatial_datatypes)
            + """')
                    THEN
                        concat('ST_AsText(',column_name,')')

                ELSE
                    concat('cast(`',column_name,'` AS char CHARACTER SET """
            + self.charset
            + """)')
                END
                AS select_csv,
        ...
        select_csv = [
            "COALESCE(REPLACE(%s, '\"', '\"\"'),'NULL') " % statement["select_csv"]
        ...
        select_columns["select_csv"] = (
            "REPLACE(CONCAT('\"',CONCAT_WS('\",\"',%s),'\"'),'\"NULL\"','NULL')"
            % ",".join(select_csv)
        )
    ...
    def begin_tx(self):
        ...
        self.cursor_unbuffered.execute(
    ...
    def make_tx_snapshot(self, schema, table):
        ...
        """
        self.logger.debug(
            "reading and discarding 1 row from `%s`.`%s`" % (schema, table)
        )
        self.cursor_unbuffered.execute(
            "SELECT * FROM `%s`.`%s` LIMIT 1" % (schema, table)
        )
    ...
    def lock_table(self, schema, table):
        ...
        self.logger.debug(
            "collecting the master's coordinates for table `%s`.`%s`" % (schema, table)
        )
    ...
    def unlock_tables(self):
        ...
        """
        self.logger.debug("START unlocking the tables")
        sql_unlock = "UNLOCK TABLES;"
        self.cursor_buffered.execute(sql_unlock)
        self.logger.debug("DONE unlocking the tables")
    ...
    def convert_batch_data(
            self, csv_results: Sequence[tuple], source_schema: str, table: str, columns,
            copy_mode=False) -> list[tuple]:
        self.logger.debug("START convert_batch_data ")

        def _apply_row_conversion(
        ...
        import pg_chameleon.lib.convert_functions as convert_functions
    ...
    def insert_table_data(self, ins_arg):
        ...
        loading_schema = self.schema_loading[schema]["loading"]
        num_insert = 1
        # TODO: add conversiont here
        for slice in slice_insert:
            self.logger.info(
                "Executing inserts in %s.%s. Slice %s. Rows per slice %s."
                % (
                    loading_schema,
                    table,
                    num_insert,
            ...
            insert_data = self.convert_batch_data(insert_data,schema,table,column_list)
    ...
    def print_progress(self, iteration, total, schema, table):
        ...
        if total > 1:
            self.logger.info(
                "Table %s.%s copied %s slice of %s" % (schema, table, iteration, total)
            )
        else:
            self.logger.debug(
                "Table %s.%s copied %s slice of %s" % (schema, table, iteration, total)
    ...
    def __copy_tables(self):
        ...
        for schema in self.schema_tables:
            ...
            for table in table_list:
                self.logger.info(
                    "Copying the source table %s into %s.%s"
                    % (table, loading_schema, table)
                )
                try:
                    if self.keep_existing_schema:
                        table_pkey = self.pg_engine.get_existing_pkey(
                            destination_schema, table
                        )
                        self.logger.info(
                            "Collecting constraints and indices from the destination table  %s.%s"
                            % (destination_schema, table)
                        )
                        self.pg_engine.collect_idx_cons(destination_schema, table)
                        self.logger.info(
                            "Removing constraints and indices from the destination table  %s.%s"
                            % (destination_schema, table)
                        )
                        try:
                            self.pg_engine.cleanup_idx_cons(destination_schema, table)
                        except Exception as drop_contraints_exception:
                            print("\n### Error when dropping constraints. Keep going...")
                            print(drop_contraints_exception)
                            print("\n###")
                        self.logger.info(
                            "Truncating the table  %s.%s" % (destination_schema, table)
                        )
                    ...
                    self.pg_engine.store_table(
                        destination_schema, table, table_pkey, master_status
                    )
                    if self.keep_existing_schema:
                        # input("Press Enter to continue...")
                        self.logger.info(
                            "Adding constraint and indices to the destination table  %s.%s"
                            % (destination_schema, table)
                        )
                        self.pg_engine.create_idx_cons(destination_schema, table)
                except Exception as exc:
                    self.logger.info(
                        "Could not copy the table %s. Excluding it from the replica."
                        % (table)
                    )
                    print( exc)
    ...
    def __init_read_replica(self):
        ...
        try:
            exit_on_error = (
                True if self.source_config["on_error_read"] == "exit" else False
            )
        except KeyError:
            exit_on_error = True
        ...
        if self.gtid_mode:
            ...
            self.start_xid = (
                master_data[0]["Executed_Gtid_Set"].split(":")[1].split("-")[0]
            )
    ...
    def __init_sync(self):
        ...
        if "keep_existing_schema" in self.sources[self.source]:
            self.keep_existing_schema = self.sources[self.source][
    ...
    def refresh_schema(self):
        ...
        try:
            ...
            notifier_message = "refresh schema %s for source %s is complete" % (
                self.schema,
                self.source,
            )
            ...
            if not self.keep_existing_schema:
                self.drop_loading_schemas()
            self.pg_engine.set_source_status("error")
            notifier_message = "refresh schema %s for source %s failed" % (
                self.schema,
                self.source,
            )
    ...
    def sync_tables(self):
        ...
        self.__build_table_exceptions()
        self.schema_list = [
            schema for schema in self.schema_mappings if schema in self.schema_only
        ]
        self.get_table_list()
        self.create_destination_schemas()
        try:
            ...
            self.pg_engine.set_source_highwatermark(master_end, consistent=False)
            self.pg_engine.cleanup_table_events()
            notifier_message = "the sync for tables %s in source %s is complete" % (
                self.tables,
                self.source,
            )
            self.notifier.send_message(notifier_message, "info")
            ...
            if not self.keep_existing_schema:
                self.drop_loading_schemas()
            self.pg_engine.set_source_status("error")
            notifier_message = "the sync for tables %s in source %s failed" % (
                self.tables,
                self.source,
            )
            self.notifier.send_message(notifier_message, "critical")
    ...
    def get_table_type_map(self):
        ...
        for schema in self.schema_replica:
            ...
            for table in table_list:
                ...
                self.cursor_buffered.execute(
                    sql_columns, (table["table_schema"], table["table_name"])
                )
    ...
    def __skip_event(self, table, schema, binlogevent):
        ...
        if self.skip_events:
            if self.skip_events[event]:
                ...
                if (
                    schema in self.skip_events[event]
                    or table_name in self.skip_events[event]
                ):
                    skip_event = True
    ...
    def convert_row_data(
        self, row: dict, source_schema: str, table_name: str
    ) -> dict:
        import pg_chameleon.lib.convert_functions as convert_functions
        if _schema := self.convert_columns.get(source_schema):
            if _table := _schema.get(table_name):
                convert_dict = {
                    k: getattr(convert_functions, v) for k, v in _table.items()
                }
    ...
    def __read_replica_stream(self, batch_data):
        ...
        if gtid_set:
            self.logger.debug(
                "GTID ENABLED - gtid: %s. id_batch: %s " % (gtid_set, id_batch)
            )
        else:
            self.logger.debug(
                "GTID DISABLED - log_file %s, log_position %s. id_batch: %s "
                % (log_file, log_position, id_batch)
            )
        ...
        for binlogevent in my_stream:
            if isinstance(binlogevent, GtidEvent):
                ...
                self.logger.debug(
                    "ROTATE EVENT - binlogfile %s, position %s. "
                    % (binlogfile, position)
                )
                if (log_file != binlogfile and stream_connected) or len(
                    group_insert
                ) > 0:
                    close_batch = True
                ...
                self.logger.debug(
                    "HEARTBEAT EVENT - binlogfile %s " % (binlogevent.ident,)
                )
                if len(group_insert) > 0 or log_file != binlogevent.ident:
                    self.logger.debug(
                        "WRITING ROWS - binlogfile %s " % (binlogevent.ident,)
                    )
                ...
                if (
                    binlogevent.query.strip().upper() not in self.statement_skip
                    ...
                    if len(group_insert) > 0:
                        ...
                        group_insert = []
                    self.logger.info(
                        "QUERY EVENT - binlogfile %s, position %s.\n--------\n%s\n-------- "
                    ...
                    for token in sql_tokeniser.tokenised:
                        ...
                        store_query = self.__store_binlog_event(
                            table_name, schema_query
                        )
                        if store_query:
                            ...
                            if table_key_dic in inc_tables:
                                ...
                                if log_seq > table_dic["log_seq"]:
                                    ...
                                    log_seq == table_dic["log_seq"]
                                    and log_pos >= table_dic["log_pos"]
                                ):
                                    write_ddl = True
                                if write_ddl:
                                    self.logger.info(
                                        "CONSISTENT POINT FOR TABLE %s REACHED  - binlogfile %s, position %s"
                                        % (table_key_dic, binlogfile, log_position)
                                    )
                                    self.pg_engine.set_consistent_table(
                                        table_name, destination_schema
                                    )
                                    inc_tables = (
                                        self.pg_engine.get_inconsistent_tables()
                                    )
                            if write_ddl:
                                event_time = binlogevent.timestamp
                                ...
                                if len(token) > 0:
                                    ...
                                    self.pg_engine.write_ddl(
                                        token, query_data, destination_schema
                                    )
                ...
                for row in binlogevent.rows:
                    ...
                    if store_row and not skip_event[0]:
                        ...
                        table_charset = table_type_map[schema_row][table_name][
                            "table_charset"
                        ]
                        ...
                        if add_row:
                            ...
                            event_before = self.convert_row_data(
                                event_before, schema_row, table_name
                            )
                            event_after = self.convert_row_data(
                                event_after, schema_row, table_name
                            )
                            for column_name in event_after:
                                try:
                                    ...
                                    self.logger.debug(
                                        "Detected inconsistent structure for the table  %s. The replay may fail. "
                                        % (table_name)
                                    )
                                    column_type = "text"
                                if (
                                    column_type in self.hexify
                                    and event_after[column_name]
                                ):
                                    event_after[column_name] = binascii.hexlify(
                                        event_after[column_name]
                                    ).decode()
                                elif column_type in self.hexify and isinstance(
                                    event_after[column_name], bytes
                                ):
                                    event_after[column_name] = ""
                                elif column_type == "json":
                                    event_after[column_name] = self.__decode_dic_keys(
                                        event_after[column_name]
                                    )
                                elif (
                                    column_type in self.spatial_datatypes
                                    and event_after[column_name]
                                ):
                                    event_after[column_name] = self.__get_text_spatial(
                                        event_after[column_name]
                                    )
                            ...
                            for column_name in event_before:
                                ...
                                if (
                                    column_type in self.hexify
                                    and event_before[column_name]
                                ):
                                    event_before[column_name] = binascii.hexlify(
                                        event_before[column_name]
                                    ).decode()
                                elif column_type in self.hexify and isinstance(
                                    event_before[column_name], bytes
                                ):
                                    event_before[column_name] = ""
                                elif column_type == "json":
                                    event_before[column_name] = self.__decode_dic_keys(
                                        event_after[column_name]
                                    )
                                elif (
                                    column_type in self.spatial_datatypes
                                    ...
                                    event_before[column_name] = self.__get_text_spatial(
                                        event_before[column_name]
                                    )
                            event_insert = {
                        ...
                        if len(group_insert) >= self.replica_batch_size:
                            ...
                            self.logger.info(
                                "Max rows per batch reached. Writing %s. rows. Size in bytes: %s "
                                % (len(group_insert), size_insert)
                            )
    ...
    def read_replica(self):
        ...
        if skip:
            self.logger.warning(
                "Couldn't connect to the source database for reading the replica. Ignoring."
            )
        else:
            ...
            if replica_paused:
                ...
                if len(batch_data) > 0:
                    ...
                    if "gtid" in master_data:
                        master_data["Executed_Gtid_Set"] = self.__build_gtid_set(
                            master_data["gtid"]
                        )
                    else:
                    ...
                    if close_batch:
                        ...
                        next_id_batch = self.pg_engine.save_master_status(
                            self.master_status
                        )
                        if next_id_batch:
                            self.logger.debug(
                                "new batch created, saving id_batch %s in class variable"
                                % (id_batch)
                            )
                            self.id_batch = id_batch
                        else:
                            self.logger.debug(
                                "batch not saved. using old id_batch %s"
                                % (self.id_batch)
                            )
                        if self.id_batch:
                            self.logger.debug(
                                "updating processed flag for id_batch %s", (id_batch)
                            )
